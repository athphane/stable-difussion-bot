...
from stablebot import StableBot, ADMINS
...
@StableBot.on_message(filters.command("history") & filters.user(ADMINS))
async def send_history_file(_, message: Message):
    await message.reply_document('activity.csv')


@StableBot.on_message(filters.command("seturl") & filters.user(ADMINS))
...
@StableBot.on_message(filters.command("geturl") & filters.user(ADMINS))
